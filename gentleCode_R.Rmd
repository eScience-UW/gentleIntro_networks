...
base + geom_node_label(aes(label = name,color=male), repel = TRUE,show.legend = F) + geom_edge_link(alpha=0.1)
```
...
base=ggraph(graph = EliteNet)
base  + geom_node_label(aes(label = label), repel = TRUE,show.legend = F,color='red') + geom_edge_link(alpha=0.1)
```
...
base + geom_node_point(aes(color=Labels), show.legend = F, size=4) +
...
```{r}
V(EliteNet)$louvain=as.factor(membership(communities))
base=ggraph(graph =EliteNet) + geom_edge_link(alpha=0.2)
base + geom_node_point(aes(color=louvain), show.legend = F, size=4) 
...
```{r}
modularity(EliteNet, partition_girvanNewman_first)
```


```{r}
modularity(partition_louvain)
```
